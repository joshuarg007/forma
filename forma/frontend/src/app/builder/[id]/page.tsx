...
import {
  ...
  Palette, Gauge, FileImage, HelpCircle, ChevronDown, ChevronUp,
  Send, MessageCircle, Rocket, Globe
} from 'lucide-react'
...
export default function BuilderPage() {
  ...
  const { currentProject, currentPage, pages, selectProject, selectPage, saveCanvasState } = useProjectStore()
  ...
  // TF.js AI Chat suggestions
  const [chatQuery, setChatQuery] = useState('')
  const [chatSuggestions, setChatSuggestions] = useState<ComponentPrediction[]>([])
  const [chatLoading, setChatLoading] = useState(false)
  const [modelStatus, setModelStatus] = useState<{ initialized: boolean; historySize: number } | null>(null)
  ...
  // Hosting store
  const { hostingConfig, fetchHostingConfig } = useHostingStore()
  ...
  // TF.js-powered chat suggestions
  const handleAskForSuggestions = async () => {
    ...
    try {
      ...
      const pageComponents = canvasComponents.map(c => c.type) as ComponentType[]

      const context: PredictionContext = {
        parentType: 'section' as ComponentType,
        existingChildren: [],
  ...
  // Add component from chat suggestion
  const handleAddFromSuggestion = (suggestion: ComponentPrediction) => {
    const newComponent: CanvasComponent = {
      id: `${suggestion.componentType}-${Date.now()}`,
      type: suggestion.componentType,
      name: suggestion.componentType.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
      props: {},
    }
    const newComponents = [...canvasComponents, newComponent]
    setCanvasComponents(newComponents)
    ...
    const context: PredictionContext = {
      parentType: 'section' as ComponentType,
      existingChildren: [],
      siblingComponents: canvasComponents.map(c => c.type) as ComponentType[],
      pageComponents: canvasComponents.map(c => c.type) as ComponentType[],
    }
    recordInteraction(context, suggestion.componentType)
  ...
  const handlePreviewInNewTab = () => {
    ...
    const previewData = {
      ...
      // Include runtime URL for data binding in preview
      runtimeUrl: currentProject?.runtime_api_url || null,
    }
...
${componentsHtml}
  ...
  // Load project and initialize TF.js model
  useEffect(() => {
    if (projectId && user) {
      selectProject(projectId)
      // Get TF.js model status
      ...
      <header className="h-14 border-b border-white/10 bg-forma-950/80 backdrop-blur-xl flex items-center justify-between px-4 flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          {modelStatus && (
            <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 border border-white/10">
              ...
              <span className="text-sm text-white/60">
                {modelStatus.initialized ? 'AI Ready' : 'Loading...'}
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-1.5 rounded-lg bg-gradient-to-r from-forma-500 to-purple-500 hover:from-forma-600 hover:to-purple-600 text-white text-sm font-medium transition shadow-lg shadow-forma-500/20"
          >

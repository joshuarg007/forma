...
interface SchemaState {
  ...
  // Actions - Backend Deployment
  saveToProject: (projectId: string) => Promise<void>
  deployBackend: (projectId: string) => Promise<DeployResult>
  ...
  persist(
    (set, get) => ({
      ...
      saveToProject: async (projectId: string) => {
        ...
        await api.put(`/api/projects/${projectId}/schema`, {
          schema: schema,
        })
      ...
      deployBackend: async (projectId: string) => {
        const { saveToProject } = get()
        ...
        const result = await api.post<DeployResult>(
          `/api/projects/${projectId}/deploy-backend`
        )
      ...
      loadFromProject: async (projectId: string) => {
        try {
          const schema = await api.get<SchemaDefinition>(
          ...
          console.log('No schema found for project, starting fresh')
        }
      },

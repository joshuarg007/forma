...
export default function FooterColumns({
  ...
  return (
    <footer
      id={id}
      className={cn(themeStyles.bg, 'py-16', className)}
      style={styles}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 md:grid-cols-6 gap-8 lg:gap-12">
          ...
          <div className="col-span-2">
            <a href="/" className="flex items-center gap-3 mb-4">
              ...
              <span
                className={cn('font-bold text-xl', themeStyles.logo)}
            ...
            <p
              className={cn('text-sm leading-relaxed mb-6 max-w-xs', themeStyles.text)}
              contentEditable={editable}
              suppressContentEditableWarning
          ...
          {/* Link Columns */}
          {links && links.map((column, index) => (
            <div key={index}>
              <h3 className={cn('font-semibold mb-4', themeStyles.heading)}>
                {column.category}
              </h3>
              <ul className="space-y-3">
                {column.items.map((link, linkIndex) => (
                  <li key={linkIndex}>
                    <a
                      href={link.href}
                      className={cn('text-sm transition-colors', themeStyles.link)}
                    >
                      {link.label}
                    </a>
                  </li>
        ...
        {/* Bottom Bar */}
        <div className={cn('mt-12 pt-8 border-t', themeStyles.border)}>
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-6">
              <a href="#" className={cn('text-sm transition-colors', themeStyles.link)}>
                Privacy Policy
              </a>
              <a href="#" className={cn('text-sm transition-colors', themeStyles.link)}>
                Terms of Service
...
FooterColumns.config = {
  id: 'footer',
  name: 'Footer Columns',
  category: 'footer',
  description: 'Full footer with link columns and social links',
  defaultProps: {
    logoText: 'Brand',
  ...
  editableFields: [
    ...
    { name: 'tagline', label: 'Tagline', type: 'textarea', defaultValue: 'Building the future of web development.' },

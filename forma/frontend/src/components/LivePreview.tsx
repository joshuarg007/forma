...
import {
  ...
  ZoomIn, ZoomOut, RotateCcw, Maximize2, X, Sun, Moon, Wifi,
  Battery, Signal, Clock, ChevronUp, Globe
} from 'lucide-react'
...
export default function LivePreview({ code, componentName, isOpen, onClose }: LivePreviewProps) {
  ...
  // Update time every minute for device status bar
  useEffect(() => {
  ...
  const generatePreviewHTML = () => {
    const processedCode = code
      .replace(/^export default /m, 'const PreviewComponent = ')
      .replace(/^export /gm, '')
...
<body>
  ...
  <script type="text/babel">
    ...
    const Target = ({ className }) => (
      <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4-1.79-4-4-4zm0-4c-4.42 0-8 3.58-8 8s3.58 8 8 8 8-3.58 8-8-3.58-8-8-8zm0 16c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8z" />
      </svg>
    );
    ...
    // Motion placeholder (just pass-through)
    const motion = {
    ...
    // Make Link just render an anchor
    const Link = ({ children, href, className, ...props }) => (
      <a href={href || '#'} className={className} {...props}>{children}</a>
    );

    // Router hooks placeholder
    ...
    root.render(React.createElement(PreviewComponent || (() => <div className="p-4 text-center">No component to render</div>)));
  </script>
</body>
</html>
`
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="bg-forma-950 rounded-2xl border border-white/10 w-full max-w-7xl max-h-[95vh] flex flex-col shadow-2xl overflow-hidden"
        >
          {/* Header */}
          <div className="flex items-center justify-between p-4 border-b border-white/10 bg-forma-950/80 backdrop-blur-xl">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1 p-1 rounded-lg bg-white/5 border border-white/10">
                {Object.entries(DEVICES).slice(0, 5).map(([key, dev]) => (
                  <button
                    key={key}
              ...
              <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-white/5 border border-white/10">
                ...
                <button
                  onClick={() => setZoom(100)}
                  className="p-1 text-white/60 hover:text-white transition ml-1"
                  title="Reset zoom"
                >
              ...
              <button
                ...
                className="p-2 rounded-lg bg-white/5 border border-white/10 text-white/60 hover:text-white transition"
              ...
              <button
                ...
                className="p-2 rounded-lg bg-white/5 border border-white/10 text-white/60 hover:text-white transition disabled:opacity-50"
              ...
              <button
                onClick={onClose}
                className="p-2 rounded-lg bg-white/5 border border-white/10 text-white/60 hover:text-white hover:bg-red-500/20 transition"
                title="Close"
              >
                <X className="w-4 h-4" />
              </button>
          ...
          {/* Preview area */}
          <div className="flex-1 overflow-auto bg-[#1a1a2e] p-8 flex items-center justify-center">
            <div
            ...
            >
              {/* Device frame */}
              {selectedDevice.includes('mobile') && (
                <div className="relative">
                  ...
                  >
                    {/* Status bar */}
                    <div className="absolute top-6 left-1/2 -translate-x-1/2 w-32 h-6 bg-black rounded-full z-10 flex items-center justify-center">
                    ...
                    >
                      {/* iOS status bar */}
                      <div className="absolute top-0 left-0 right-0 h-12 bg-black/5 dark:bg-white/5 flex items-center justify-between px-8 z-10">
                      ...
                      {/* Home indicator */}
                      <div className="absolute bottom-2 left-1/2 -translate-x-1/2 w-32 h-1 bg-gray-300 dark:bg-gray-700 rounded-full" />
                    </div>
                  </div>
                </div>
              )}

              {selectedDevice.includes('tablet') && (
                <div className="relative">
                  {/* Tablet frame */}
                  <div
                    className="bg-gray-900 rounded-[2rem] p-4 shadow-2xl"
              ...
              {(selectedDevice === 'desktop' || selectedDevice === 'laptop') && (
                <div className="relative">
                  ...
                  <div className="bg-gray-800 rounded-t-xl p-1 shadow-2xl">
                    ...
                    <div className="bg-gray-700 rounded-t-lg px-4 py-2 flex items-center gap-2">
                      ...
                      <div className="flex-1 mx-4">
                        <div className="bg-gray-600 rounded-md px-4 py-1.5 text-gray-400 text-sm flex items-center gap-2">
                          <Globe className="w-4 h-4" />
                          <span>preview.forma.app/{componentName.toLowerCase().replace(/\s+/g, '-')}</span>
                        </div>
                      </div>
                    </div>
                  ...
                  {selectedDevice === 'desktop' && (
                    <>
                      <div className="mx-auto w-24 h-16 bg-gray-700" />
                      <div className="mx-auto w-48 h-3 bg-gray-800 rounded-b-lg" />
                    </>
                  )}
                </div>
              )}
          ...
          {/* Device info bar */}
          <div className="px-4 py-2 border-t border-white/10 bg-forma-950/80 flex items-center justify-between text-sm">
            <div className="flex items-center gap-4 text-white/60">
              <span>{device.name}</span>
              <span>{device.width} × {device.height}</span>
            </div>
            <div className="flex items-center gap-2 text-white/40">
              <span className={darkMode ? 'text-blue-400' : 'text-yellow-400'}>

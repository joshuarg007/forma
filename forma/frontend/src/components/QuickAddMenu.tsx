...
export default function QuickAddMenu({ isOpen, onClose, onSelect, existingComponents, position }: QuickAddMenuProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Use TensorFlow.js model for smarter predictions
      const fetchAIPredictions = async () => {
        try {
          ...
          const context: PredictionContext = {
            parentType: 'section' as ComponentType, // Default parent for root-level additions
            existingChildren: [],
            siblingComponents: existingComponents as ComponentType[],
            pageComponents: existingComponents as ComponentType[],
          }

          const aiPredictions = await getSmartSuggestions(context, 8)
          setAiReady(true)

          if (aiPredictions.length > 0) {
            // Merge AI predictions with our component data
            const enhanced = aiPredictions.map(pred => {
              const base = allComponents.find(c => c.id === pred.componentType)
              if (base) {
                return {
                  ...base,
          ...
          console.warn('[QuickAddMenu] TF.js prediction failed, using rule-based:', e)
        } finally {
          setLoading(false)
          setUsingAI(false)
        }
  ...
  const handleSelect = (item: QuickAddItem) => {
    // Record interaction for TF.js model learning
    const context: PredictionContext = {
    ...
    recordInteraction(context, item.id as ComponentType)
      ...
      >
        ...
        <div className="px-4 py-3 bg-gray-50 border-t border-gray-100">
          <p className="text-xs text-gray-500 text-center">
            ...
            {aiReady && <span className="ml-2 text-green-600">✨ Learns from your choices</span>}
          </p>
        </div>
      </motion.div>
    </>
